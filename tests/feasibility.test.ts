...
/** The table in docs/PLAN.md §4.1, as a test — so the documented limits and
 * the shipped ones cannot drift apart. Ten factions, dealt format. */
describe("what the dealt format can actually deal", () => {
  const cases: Array<[players: number, bans: number, maxPool: number]> = [
    [2, 0, 5],
    [3, 1, 2],
    [4, 0, 2],
    [4, 1, 1],
  ...
  it.each(cases)("%i players with %i ban(s) each can be offered %i", (players, bans, maxPool) => {
...
describe("feasibility explains itself", () => {
  it("names the arithmetic when the pool will not fit", () => {
    ...
    expect(issue.vars).toMatchObject({ players: 6, pool: 2, need: 12, left: 10, max: 1 });
  });
  ...
  it("says when the bans have eaten the table", () => {
    ...
    expect(check.factionsAfterBans).toBe(0);
  });
  ...
  it("counts towns, not cards, when two heroes may not share a faction", () => {
    ...
    // towns; there are ten.
    expect(feasibility(config).maxHeroPoolSize).toBe(2);
  });
});
