...
Eleven factions as the catalogue stands, the tenth official one plus the
unofficial Factory:
...
have 11 — switch to Snake, or reduce the pool"). This is a real limit of the
...
**Factions (11 in the catalogue, 13 official ones announced).** Castle, Rampart, Tower, Inferno,
...
- **The unofficial Factory expansion**, read off the hero boards in its own
  ...
  classes, abilities and specialties came off the cards; its stat strips were
  cut from the same boards and composited to the exact shape of the official
  ones, which is possible because base statistics depend only on faction and
  class, so two images cover seven heroes. Its crest is a cog, generated by
  arithmetic rather than drawn by eye — eight teeth stay even that way.

  This turned `wiki` from a page name into a whole URL. There is no longer one
  base to bolt a slug on to: the official ten point at the community database
  and Factory points at its own repository.
